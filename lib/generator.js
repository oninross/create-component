...
// Utility to convert PascalCase to camelCase
...
export async function generateComponent({
  ...
  const componentNameCamel = pascalToCamel(componentName);
  const styleImport =
    styling === "SCSS Module"
  ...
  const rootElement =
    styling === "SCSS Module"
      ? `<div className={styles.${componentNameCamel}}>{children}</div>`
      : `<S.${componentName} className={${componentNameCamel}}>{children}</S.${componentName}>`;
